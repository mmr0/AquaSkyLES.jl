...
for n in 1:length(QSH.times)
    ...
    ΔT[n] = -mean(θt[:,1,2,n])+parameters.sea_surface_temperature
...
axΔT = Axis(fig[3, 1], ylabel=L"\Delta T ~(\mathrm{K})", xlabel = "time [h]")
